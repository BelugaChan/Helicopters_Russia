...
using Helicopters_Russia.Abstract;

namespace Helicopters_Russia.Algotithms
{
    public class CosineSimAlgo : SimilarityCalculator
    {
        ...
        public override (List<TGarbageData> worst, Dictionary<TGarbageData, (Dictionary<TStandart, double>, string)> mid, Dictionary<TGarbageData, Dictionary<TStandart, double>> best) CalculateCoefficent<TStandart, TGarbageData>(AlgoResult<TStandart, TGarbageData> algoResult)
        {
            ...
            //we'll rework this
            var allProcessedStandarts = new ConcurrentBag<TStandart>(processedStandarts.GroupStandarts.Values.SelectMany(list => list));
            ...
            where TGarbageData : IGarbageData
        {
            Parallel.ForEach(matchedData, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, (item, state) =>
            {
                ...
                foreach (var standartGroups in standartStuff) //сравнение грязной строки со всеми позициями каждой из групп, где хотя бы в одном из элементов совпал гост с грязной позицией. Чаще всего количество сопоставленных групп - 1.
                {
                    ...
                    {
                        var similarity = /*cosine*/cosineSimpled.Similarity(improvedProcessedGarbageName, standart.HandledName/*standart.Value*/);//основной алго

                        //выделение уникальных чисел для позиции эталона
                        var standartGosts = new HashSet<string>() { standart/*.Key*/.UpdatedMaterialNTD, standart/*.Key*/.UpdatedNTD };
        ...
        public void DefaultRun<TStandart,TGarbageData>
            ...
            ConcurrentBag<TStandart> allProcessedStandarts,
            ConcurrentBag<TGarbageData> worstBag, 
            ConcurrentDictionary<TGarbageData,(Dictionary<TStandart, double>,string)> midBag, 
            ConcurrentDictionary<TGarbageData, Dictionary<TStandart, double>> bestBag)
            where TStandart : IStandart
            where TGarbageData : IGarbageData
        {
            Parallel.ForEach(dataForPostProcessing, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, (item, state) =>
            {
                ...
                double similarityCoeff = -1;
                foreach (var standart/*(standart, standartName)*/ in allProcessedStandarts)
                {
                    double coeff = /*cosine*/cosineSimpled.Similarity(garbageName, standart.HandledName/*standartName*/);
